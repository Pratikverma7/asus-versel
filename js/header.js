        ...
        fetch(`${baseUrl}/products?is_rog=0&search=${searchValue}&limit=10`)
            ...
            .then(data => {
                const productHTML = data.data.map((product) => `
                    <div style="display:flex;justify-content:space-between;border-bottom:1px solid #e6e6e6; padding: 6px 0px;">
...
function handleBuy(url){
  ...
  .then(data => {
     ...
     if(linksContainer){
        linksContainer.innerHTML = `${seriesData.map(series =>`<li class="link-class"><a href="listing.html?series=${series.id}&${series.name}">${series.name}</a></li>`).join('')}`;
     }
     let mobileLinksContainer = document.getElementById('mobile-links-container');
     if(mobileLinksContainer){
        mobileLinksContainer.innerHTML = `${seriesData.map(series =>`<li class="mobile-links-class"><a href="listing.html?series=${series.id}&${series.name}">${series.name}</a></li>`).join('')}`;
     }
  })
  .catch(error => {
  ...
  function handleHamburger() {
    ...
    let isHidden = window.getComputedStyle(menuContent).display === 'none';
    ...
    if (isHidden) {
      ...
      html.style.overflow = ''; // Restore scrolling
    }
  }
  
  

  function handleCatRoute(id,name){
    window.location.href = `/listing.html?child_series_id=${id}&child_series_name=${name}`;
  }